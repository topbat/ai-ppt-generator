...
“新建生成”和“PPT-MASTER 生成”均从后端环境配置读取可选模型，并要求用户在 `deepseek-v4`、`qwen3.7-plus`、`qwen3.8-max` 中选择一个。PPT-MASTER 的模板填充路线由上传的 PPTX 决定视觉风格；任务列表展示模型与完整阶段轨迹；生成任务最多同时运行三个，其余任务继续接受并排队。
