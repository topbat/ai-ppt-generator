...
### 3.1 企业模板空间契约与布局语法（已实现）

模板解析器 v6 为每个模板页生成 `TemplateSpaceContract`：页面尺寸、带四边内距的正文安全区、logo/页脚保护区、12列网格和 gutter。渲染优先使用该安全区；标题几何异常（例如整页背景误识别为标题）会在契约入口被拒绝并使用保守默认区域。

标准/专业模式在内容与渲染之间新增 14 套布局语法，分属 `single_focus / split / cards_grid / flow / structure / data_focus / editorial` 七个家族。语法使用 0..1 归一化坐标，运行时投影到模板安全区；文本容器至少保留上下 0.18in、左右 0.22in 内距，并在绘制前执行页面边界、四边内距和保护区碰撞检查。

内容容量处理顺序固定为：保留结论/事实 → 保留主要解释 → 把支持材料移动到演讲者备注。不会通过缩小正文到 16pt 以下解决溢出。迁移前后页面数字集合必须一致，所有核验数字和受保护专有名称必须仍然存在。
